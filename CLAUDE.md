...
0.155.0 起 Codex 引入 standalone daemon：远端安装从 npm 迁移为 `$CODEX_HOME/packages/standalone/` 下的官方 release 包（`server/utils/gateway/infra/codex/codex-artifacts.ts` 从 `releases.openai.com`/GitHub release 下载并校验 sha256），`server/utils/gateway/infra/codex/codex-version-checker.ts` 额外探测 `installationLayout`（`standalone` vs `npm-or-external`），非 standalone 布局即使版本号匹配也会触发一次真实重装——`deploy/user-container` 镜像里 `npm install -g @openai/codex` 装出来的仍是 npm 布局，因此**每个新建的用户容器在第一次真正连接时都会经历一次到 standalone 布局的强制迁移**（`server/utils/gateway/infra/rpc/rpc.ts` 的 `CodexRpcClient.connect()` 保证这次迁移发生在 app-server 真正启动之前），这是预期行为，不是需要修的 bug。

app-server 启动方式同时改为 `nohup "$CODEX_BIN" app-server daemon bootstrap --remote-control`（`server/utils/gateway/infra/ssh/remote-command.ts`），`daemon bootstrap` 要求 `$CODEX_HOME/packages/standalone/current/codex` 必须存在，否则直接报错拒绝启动（已用 0.155.0 实测确认）——这进一步说明上面那次 standalone 迁移是硬性前置条件，不只是版本对齐的优化。`--remote-control` 参数会让 remote control 直接进入 `EnabledEphemeral` 状态（`codex-rs/app-server/src/main.rs`），跳过没有 ChatGPT 登录时那条每秒重试一次的 `resolve_unknown_desired_state` 解析循环（`codex-rs/app-server-transport/.../remote_control/websocket.rs`），改用封顶 30 秒的指数退避重连；已用 0.155.0 实测验证（直接以 `--remote-control --listen unix://` 启动、无 ChatGPT 登录，观察 `~/.codex/logs_2.sqlite` 55 秒内退避从约 0.2s 递增到 30s 触顶后重置，不是历史上"每秒一条"的刷屏）。因此 `deploy/user-container/entrypoint.sh` 里原本用来关闭 remote control 的 `CODEX_INTERNAL_APP_SERVER_REMOTE_CONTROL_DISABLED`（`--remote-control` 优先于该内部环境变量，见 main.rs）在 Gateway 自己的启动路径下已不再起作用，entrypoint.sh 现在只清理旧镜像遗留的 profile.d 文件，不再设置这个变量。

### 出站代理

`CODEX_GATEWAY_OUTBOUND_PROXY`（可选 `CODEX_GATEWAY_OUTBOUND_NO_PROXY`）是唯一的出站代理配置项，不设置时行为完全不变。覆盖两条独立路径：

- **Gateway 进程自身**：`deploy/gateway-entrypoint.sh` 是镜像的 ENTRYPOINT，只在这个变量非空时把它转换成 Node 需要的小写 `http_proxy`/`https_proxy`/`no_proxy` + `NODE_USE_ENV_PROXY=1` 再 `exec node`。Node 原生 `fetch()`（`codex-artifacts.ts` 的 Codex 发行包下载、`codex-latest.get.ts` 的 npm 版本检查、`bark-provider.ts` 的推送）默认不读这些变量，`NODE_USE_ENV_PROXY=1` 让它经 undici 的 `EnvHttpProxyAgent` 读取——但**这个开关必须在进程启动时就在环境变量里，进程起来后再用 JS 设置 `process.env` 完全无效**（已用 `node:24-bookworm-slim` 实测：同样的变量传给 `docker run -e` 生效，传给已运行进程内的 `process.env.x = y` 不生效），这是选择"入口 shell 包装脚本"而不是"在 Nitro 插件里读设置"的直接原因。另外只有小写变量生效，大写 `HTTP_PROXY`/`HTTPS_PROXY` 不生效（同样实测确认）。Codex 发行包是 **Gateway 自己下载后经 SSH 上传**给远端主机/用户容器（`codex-upgrader.ts:installOnce` → `uploadFileResumable`），不是主机自己下载，所以这一个配置项就覆盖了升级链路，不需要在远端也配代理。
