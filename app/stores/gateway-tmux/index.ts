...
export const useGatewayTmuxStore = defineStore("gateway-tmux", () => {
  ...
  async function loadSummary(force = false) {
    if (!force && loadedAt.value !== null && Date.now() - loadedAt.value < 30_000) return;
    if (pendingSummary !== null) {
      ...
      if (pendingSummary !== null && pendingSummary !== previousRequest) {
  ...
  return {
    panelOpen: skipHydrate(panelOpen),
    active,
    history,
    loading,
    error,
