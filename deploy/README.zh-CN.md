...
Gateway 自己下载 Codex standalone 发行包（约 130MB/版本/平台）再经 SSH 上传给远端主机和用户容器。这份包按
`<版本>/<资产名>` 缓存在 `CODEX_GATEWAY_DB_PATH` 同级的 `codex-artifacts/` 目录下——也就是持久卷 `/data` 上，因此**所有主机、
所有新建用户容器、Gateway 重启后都复用同一份**，不再每次重下。要挪位置用 `CODEX_GATEWAY_CODEX_ARTIFACT_CACHE_DIR`。

几个行为约定：

- 命中缓存前仍然**按 release 元数据里的 sha256 重新校验**，校验不过就当未命中重新下载，缓存文件损坏不会被用上。
- 下载写到同目录的 `.part` 临时名，校验通过后 `rename` 原子发布，并在旁边写一份 `.sha256`。元数据暂时拉不到（代理不通）时，
  就用这份记录下来的摘要校验已有缓存继续安装——跳过的只是那次网络请求，校验本身不跳过。
- 只保留最近 2 个版本，升级后旧版本自动清掉；超过 6 小时的 `.part` 残留（进程崩溃留下的）也会顺带清理。
- **不进备份**：缓存目录和 `backups/` 是同级的兄弟目录，`runBackup()` 和 `deploy/scripts/backup.sh` 都不会带上它。
- 整个目录**随时可以直接删**，下次安装会重新下载。
